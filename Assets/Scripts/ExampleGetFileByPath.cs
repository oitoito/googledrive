...
public class ExampleGetFileByPath : AdaptiveWindowGUI
{
    ...
    private IEnumerator GetFileByPathRoutine (string filePath)
    {
        ...
        Debug.Log("GetFileByPathRoutine 0");

        if (parentNames != null)
        {
            Debug.Log("parentNames not NULL");
        ...
        {
             Debug.LogFormat("FOLDERS {0} \t {1}", filePath, request.ResponseData.Files[i] );
        }

       // request.ResponseData.F
        Debug.LogFormat("FOLDERS {0} \t {1}", filePath, request.ResponseData.Files[0]   );
        if (request.ResponseData.Files.Count > 1)
        ...
        result = string.Format("ID: {0} Size: {1:0.00}MB Modified: {2:dd.MM.yyyy HH:MM:ss}",
            ...
            */
        Debug.Log("GetFileByPathRoutine 1");

        request = new GoogleDriveFiles.ListRequest();
        request.Q   = $"'{parendId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false";
        ...
        if (request.ResponseData.Files.Count > 1)
                Debug.LogFormat("We have so many files here {0}", request.ResponseData.Files.Count);

            for(int i = 0; i < request.ResponseData.Files.Count; i++)
            {
                //var file = request.ResponseData.Files[i];

                //Debug.LogFormat("i {0} \t {1} \t {2}", i, file.Id, file.name);
                Debug.LogFormat("i {0} \t {1} \t {2}", i, request.ResponseData.Files[i].Id, request.ResponseData.Files[i].Name);
//                Debug.LogFormat("i {0} \t {1} \t {2}", i, filePath, request.ResponseData.Files[i]);
